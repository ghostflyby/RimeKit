...
protocol SwiftDataSized {
    associatedtype CType: CDataSized
    
    func toCStructure(with c:inout CType)
}

extension SwiftDataSized {
    func toCStructure() -> CType {
        var c=CType.rimeStructInit()
        toCStructure(with: &c)
        return c
    }
}
