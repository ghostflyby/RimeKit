...
- **V8 勘误与解锁(2026-09-10,iOS 构建阻塞根因改判)**:V7 及会话记录中"IRGen signal 5 由方法规模(80+)触发、与部署目标无关"的判断**均不成立**。经约 20 组二分/对照实验(swift-frontend 单文件探针循环,直接复用崩溃日志命令,~40s/组)收敛出必要且充分的触发条件,**独立最小包已复现**:
  ...
  - **解锁(两项均实证,已采纳前者)**:
    1. **一行修复**:`ObjectHandle<T: ~Copyable>` 去除 `~Copyable` 约束(RimeConfig.swift:8)。该约束从未被利用——全部实例化实参(`RimeConfig`/`RimeConfigIterator`/`RimeCandidate`)均可拷贝,纯声明性意图标记。iOS 16 floor 保持不变;真实仓库 ios16 destination 整包(含 `RimeKitRimeService`)构建回绿,macOS 构建不受影响。
    2. 备选:Package.swift iOS floor 16→18(零源码改动,实测回绿;代价是放弃 iOS 16/17 设备)。未采纳。
  - swiftlang 上报素材就绪:`Docs/irgen-633-min-repro/`(独立包,不依赖 RimeKit/librime;核心 12 行:1 个 distributed actor + 1 个返回 `NCBox<T: ~Copyable>?` 的方法;macosx13 floor 下 `swift build` 即崩,`T: Sendable` 即过)。崩溃点:`IRGenModule::emitAccessibleFunction → getTypeRef → createTypeMetadataAccessFunction → createDirectTypeMetadataAccessFunction`(回调帧自递归,signal 5/SIGTRAP)。
  - F3(signal 11,typed throws 见证)与隐式 async 丢失(类型检查阶段)是否与本触发条件同源,待上报后核实;本勘误不推翻 F1/F2/F7 探针结论。另:`swift test` 在 HEAD(1301228)因 `RimeKitTests.swift:75` 引用内联重构后已删除的 `engine` 符号而无法编译,系既有遗留,与本项无关。
