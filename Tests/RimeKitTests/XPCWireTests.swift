...
// file, You can obtain one at https://mozilla.org/MPL/2.0/.
  ...
  struct XPCMarshalRoundTripTests {
    ...
    @Test func statusRoundTrip() throws {
      ...
      #expect(
        restored.schemaID == original.schemaID && restored.isSimplified == original.isSimplified)
    }
    ...
    @Test func locationSchemaAndNotificationRoundTrip() throws {
      ...
      if case .deploy = try RimeNotificationType.unmarshal(
        from: RimeNotificationType.deploy.marshal())
      {
      } else {
        Issue.record("deploy 还原失败")
      }
        ...
        from: RimeNotificationType.unknown("x").marshal())
      {
        ...
        Issue.record("unknown 还原失败")
      }

      #expect(try RimeState.unmarshal(from: RimeState.on.marshal()) == .on)
      #expect(
        try RimePageDirection.unmarshal(from: RimePageDirection.backward.marshal()) == .backward)
    }
  }
#endif
